...
const Apply = () => {
       ...
       <div className='job-deatils-heading'>
        ...
        <div>
            <ul className="custom-list" itemType='circle'>
              {
                location.state.JobList?.JobDescription?.map((data,index)=>{
                  return(
  ...
  {location.state?.JobList?.JobSummary?.length > 0 && (
    <>
      <h1 className='job-summary-heading'>Job Summary:</h1>
      <div className='job-summary-text'>
      {location.state.JobList.JobSummary.map((summary, index) => (
...
<div className='Role'>
{location.state?.JobList?.RoleOverview?.length > 0 && (
    <>
      <h1 className='job-Role-heading'>RoleOverview:</h1>
      <div className='job-Role-text'>
      {location.state.JobList.RoleOverview.map((Role, index) => (
        <p key={index}>{Role}</p>
      ))}
      </div>
    </>
...
{location.state?.JobList?.KeyResponsibilities && location.state.JobList.KeyResponsibilities.length > 0 && (
    <>
      <h1 className='key-responsibilities-heading'>Key Responsibilities:</h1>
      
      <ul>
        {location.state.JobList.KeyResponsibilities.map((responsibility, index) => (
...
<div className='qualification'>
  {location.state?.JobList?.Qualifications?.length > 0 && (
    <>
      <h1 className='qualfication-heading'>Qualifications:</h1>
      <ul className="custom-list-point" itemType='circle'>
        {location.state.JobList.Qualifications.map((data, index) => (
          <li key={index}>{data}</li>
        ))}
      </ul>
    </>
        ...
        <div className='job-deatils-heading'>
          ...
          <ol className="custom-ol-point" itemType='number'>
              {
                location.state.JobList?.Requirements?.map((data,index)=>{
                  return(
                    <li key={index}>{data}</li>
                  )
                })
      ...
      <div className='p-8  job-apply-form'>
        <div className='d-flex justify-content-between input-name-div '>
        <label>First Name 
        ...
        <div className='d-flex Contact-div '>
         ...
         <input className='input-contact' type="" value={selectedCode}  required/>
          <svg aria-hidden="true" className="e-font-icon-svg e-eicon-caret-down drop-down-icon" viewBox="0 0 571.4 571.4" xmlns="http://www.w3.org/2000/svg"><path d="M571 393Q571 407 561 418L311 668Q300 679 286 679T261 668L11 418Q0 407 0 393T11 368 36 357H536Q550 357 561 368T571 393Z"></path></svg>
        ...
        <div className="dropdown-list">
          {countryCodes.map((country, index) => (
            <div 
              ...
              onClick={() => handleSelect(country.code,country.abbreviation)}
            >
          ...
          <label className='telephone-lable'>
          Personal Telephone 
        <input className='input-telephone' type=""  required/>
        </label>
          </div>
        </div>
        ...
        {/* Upload */}
        <div>
          <label className='Upload'>Upload CV </label>
